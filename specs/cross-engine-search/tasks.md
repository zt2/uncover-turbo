...
- **里程碑 A(✅ 已完成)**:T0、T1、T2、T3–T6、T7、T10、**T11(`SearchIR`)**、T12、**T13(`-ir` 机器入口)**、T15A 终验。
  产物:一个**不依赖 LLM**、可通过 `-ir` 直接注入 IR 的跨引擎查询/聚合/渲染工具。
- **里程碑 B(✅ 已完成)**:T8、T9、T11 的 `SearchNL`、T13 的 `-q`(自然语言入口)、T14 README、T15B 终验。
  产物:补上「自然语言 → IR」的 LLM 上层(go-openai + 自定义 BaseURL,兼容 OpenRouter);删除旧 `pkg/bot`。
